...
partition <- createDataPartition(target.total_her2, p = 0.75, list = F)
print(partition)

print("Pre c50")
results <- C5.0(total_her2_data[partition, ], target.total_her2[partition], trials = 10, rules = F)
summary(results)
predictions <- predict(results, total_her2_data[-partition, ])
print(sum((predictions - target.total_her2[-partition]) == 0))
