...
for train, test in kf.split(expr_data, expr_target):
    ...
    X_train, X_test = RobustScaler().fit_transform(expr_data.values[train]), RobustScaler().fit_transform(expr_data.values[test])
...
total_her2_expr.iloc[:, :-3] = RobustScaler().fit_transform(total_her2_expr.iloc[:, :-3])

total_her2_expr.to_csv('../results/robust_rf_genes_TPM.tsv.gz', sep='\t', index=False, compression='gzip')
