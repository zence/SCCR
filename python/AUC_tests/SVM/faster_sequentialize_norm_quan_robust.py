import run_norm_quan_robust_svm
import math
import itertools
...
def sequence(genes, best_string='', best_auc=0, ix=0, *args):
    for i in range(len(genes)):
        if i >= ix:
            ...
            cur_auc = run_norm_quan_robust_svm.run(temp_genes)
...
scramble([*just_ERBB2, *spec_genes, *all_genes], add_genes=just_ERBB2)
