...
def sequence(genes, *args):
    ...
    for i in range(len(genes)):
        ...
        cur_auc = run_rf_robust.run(temp_genes)
...
all_genes = [x for x in all_genes if x is not None and x != 'Sample' and x.rstrip() != 'her2_status_by_ihc']
...
print("AUC: " + str(run_rf_robust.run([*just_ERBB2, *spec_genes, *all_genes])))
