...
def sequence(genes, best_string='', best_auc=0, ix=0, *args):
    for i in range(len(genes)):
        if i >= ix:
            ...
            cur_auc = run_rf_linear.run(temp_genes)
...
def just_one(genes):
    ...
    best_string = ''
    for i in range(len(genes)):
        cur_auc = run_rf_linear.run([genes[i]])
        print(genes[i] + ': ' + str(cur_auc), flush=True) 
        if cur_auc > best_auc:
            best_auc = cur_auc
            best_string = genes[i]
...
#scramble([*just_ERBB2, *spec_genes, *all_genes])
just_one([*just_ERBB2, *spec_genes, *all_genes])
